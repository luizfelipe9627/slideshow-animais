...
export default class SlideNav extends Slide {
  ...
  constructor(slide, wrapper) {
    super(slide, wrapper); // O super é usado para referenciar o construtor da classe pai, no caso a classe Slide.

    this.bindControlEvents(); // Executa o método bindControlEvents que é responsável por fazer o bind para referenciar o this do objeto da classe SlideNav ao invés do this do objeto da classe Slide.
  }

  // Método responsável por trocar o slide.
  ...
  createControl() {
    const control = document.createElement("ul"); // Cria um elemento HTML do tipo ul e armazena na constante control.
    ...
    this.slideArray.forEach((item, index) => {
      control.innerHTML += `<li><a href="#slide${index + 1}">${
        index + 1
      }</a></li>`; // Cada vez que o loop é executado adiciona um elemento HTML do tipo LI dentro do elemento HTML do tipo ul criado anteriormente.
    });
